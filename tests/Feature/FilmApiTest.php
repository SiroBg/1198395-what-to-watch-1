...
describe('GET api/films (index)', function () {
...
describe('GET api/films/{film} (show)', function () {
...
describe('POST api/films (store)', function () {
    test(
        'проверка доступа к созданию фильма',
        function (Closure $user, ?int $status) {
            ...
            if ($status) {
                ...
                expect($response)->assertCreated()
                    ->and($response->json('data'))->imdb_id->toBe($imdbId);
            }
        }
    )->with('film access matrix');
});

describe('PATCH api/films/{film} (index)', function () {
    test(
        'проверка доступа к редактированию фильма',
        function (Closure $user, ?int $status) {
            $film = Film::factory()->create();
            $expectedData = [
                'imdb_id' => 'tt1234567',
