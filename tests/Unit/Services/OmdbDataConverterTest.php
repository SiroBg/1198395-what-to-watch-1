...
describe('OmdbDataConverter: Базовая конвертация', function () {
    ...
    it('возвращает null или пустые массивы, если ключи отсутствуют', function () {
...
describe('OmdbDataConverter: Парсинг строк (Actors, Directors, Genres)', function () {
    it('игнорирует лишние пробелы и пустые элементы при разделении запятыми', function () {
        $omdbData = [
            'Actors' => '  Leo  ,   Tom , , Brad ',
        ];
...
describe('OmdbDataConverter: Парсинг года выпуска (Released)', function () {
    ...
    it('извлекает четырехзначный год регулярным выражением, если Carbon падает на сложной строке', function () {
        $omdbData = ['Released' => 'Фильм вышел в 1998 году'];
        expect($this->converter->convert($omdbData)['released'])->toBe(1998);
    });

    it('возвращает null, если дата равна N/A или не содержит год вообще', function () {
        expect($this->converter->convert(['Released' => 'N/A'])['released'])->toBeNull()
            ->and($this->converter->convert(['Released' => 'Not Available'])['released'])->toBeNull();
    });
});
